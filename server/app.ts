...
// Export video with uploaded media file (multipart/form-data)
app.post("/api/export-video", videoUpload.single("media"), (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ ok: false, error: "Invalid 'captions' JSON format" });
    return;
  }
  ...
  const lastCaptionEndMs = Math.max(...captions.map((c: any) => (typeof c?.endMs === "number" ? c.endMs : 0)));
  const captionDurationFrames = lastCaptionEndMs > 0 ? Math.ceil((lastCaptionEndMs / 1000) * 30) : 150;
  const rawDuration = Number(req.body.durationInFrames);
  const durationInFrames = Math.max(
    !isNaN(rawDuration) && rawDuration > 0 ? rawDuration : 0,
    captionDurationFrames
  );

  const rawScale = Number(req.body.scale);
  const scale = !isNaN(rawScale) && rawScale > 0 ? rawScale : 1;
  ...
  const body: VideoRequestBody = {
    ...
    mediaSize: file.size,
    captions,
    styleVariant: req.body.styleVariant,
    styleOverrides: parseJsonField(req.body.styleOverrides),
    overlaySettings: parseJsonField(req.body.overlaySettings),
    frameSettings: parseJsonField(req.body.frameSettings),
    textureSettings: parseJsonField(req.body.textureSettings),
    motionSettings: parseJsonField(req.body.motionSettings),
    videoMotion: parseJsonField(req.body.videoMotion),
    assetSettings: parseJsonField(req.body.assetSettings),
    audioAmplitude: parseJsonField(req.body.audioAmplitude),
    durationInFrames,
    scale,
    orientation: req.body.orientation === "horizontal" ? "horizontal" : "vertical",
  };

  const jobId = randomUUID();
  exportJobs.set(jobId, { status: "rendering", progress: 0 });

  void runVideoRender(jobId, body, requestId);
